...
- `use_valkey` (Boolean) Whether to use Valkey instead of Redis as the backend. When set to `true`, the provider will use the Valkey Glide client. When set to `false` or omitted, the provider will use the Redis client (default behavior).
